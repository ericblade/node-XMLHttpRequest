  ...
  this.send = function(data) {
    ...
    if (settings.async) {
      ...
      var responseHandler = function responseHandler(resp) {
        ...
        // Check for redirect
        // @TODO Prevent looped redirects (in a better way than just preventing > 4 redirects)
        if (response.statusCode === 301 || response.statusCode === 302 || response.statusCode === 303 || response.statusCode === 307) {
          // Change URL to the redirect location
          setState(self.HEADERS_RECEIVED);
          if (++redirectCount > 4) {
            throw new Error("XMLHttpRequest: Too many redirects");
          }
          ...
          // TODO: newOptions should go through the exact same validation
          // starting ~line 334 instead of going through this mish-mash
          ...
          var newOptions = {
            ...
            port: url.port,
            path: url.path,
            method: response.statusCode === 307 ? settings.method : "GET",
            headers: headers,
            withCredentials: self.withCredentials
          };
          ...
          if (data && newOptions.method === "POST") {
            request.write(data);
          }
